...
- React + TypeScript, built and hosted on Lovable
- Backend: Lovable-managed (NOT a separate Supabase)
- Auth: email/password only (Google/MS/SSO intentionally removed)
- GitHub: synced to Lovable on `main`
- AI in-app: Lovable's built-in AI layer
- **Scraping/enrichment: Firecrawl** (already connected to Lovable — use it)
- **Email outreach: SmartLead** (onboarded, integration to build)
- Live data: Apify, US Census, BLS, Federal Reserve (Feature 1)
