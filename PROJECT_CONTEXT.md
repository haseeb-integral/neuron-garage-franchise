...
> Snapshot date: May 21, 2026 (v1.2 two-pool architecture shipped — Master Pool vs SmartLead split; `EmailOutreachV2` now has scope switcher + 6-card stat strip with Show Formula popovers; new Master Pool Import Wizard with AI-suggested CSV mapping via `csv-suggest-mapping` edge function; Push-to-SmartLead banner + modal with dry-run preview powered by `smartlead-push-leads`; `prospect_batches` renamed → `teacher_import_batches` with `destination` / `column_mapping` / `unmapped_columns` columns; new `enrichment_jobs` table; `teacher_prospects` extended with `first_name` / `last_name` / `enrichment_cost_cents` / `enrichment_provider` / `import_batch_id` / generated `dedupe_key`. Earlier May 19: Ask AI absolute-weights mode; SmartLead Phases 1–5 complete; legacy `cities` / `city_category_scores` / `city_fetch_jobs` / `city_competitors` tables dropped.)
