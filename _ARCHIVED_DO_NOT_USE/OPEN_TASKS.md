...
- **Spreadsheet page-size dropdown:** expanded from 25/50/100 to **25/50/100/250/500/All** so the full 817-city list can be reviewed on a single page. "All" renders all rows with no virtualization (modern browsers handle ~800 rows × ~25 columns fine). Dropdown trigger widened from `w-[110px]` to `w-[130px]` to fit wider labels.
- **Retired `summer_camp_count` from the UI:** "Camps (count)" column was rendering `—` for all 817 cities (column was 0/817 populated). CSI is fully owned by the three Manus-precomputed columns (`csi_national_brand_count_weighted`, `csi_local_provider_estimate`, `csi_demand_adjusted_market`, all 817/817). Removed: spreadsheet column, `MarketReportModal` seeded `competitor_count` row, `MarketDetailDrawer` legacy `(competitor_count × 3) − (stem_camp_count × 1.5)` formula (replaced with the real Manus CSI description), `signalAliases.ts` `competitor_count → summer_camps_per_10k_children` alias, `summer_camp_count` from `loadLiveRankedMarkets` SELECT + Excel export `DASHBOARD_DB_KEYS`, and the orphan `campCount`/`campsPer10k` derivation in `buildSeededFallbackSignalsFromScored`. `RankedMarket.competitorCount` and `cityRow.competitor_count` kept as `null` (legacy shape used by sample data + `Spec.tsx`). DB column `us_cities_scored.summer_camp_count` preserved (dormant).
