...
> Snapshot date: May 22, 2026 (City universe locked at **817** — 160 non-Manus rows deleted from `us_cities_scored` after verifying 0 teacher prospects were FK-linked. Live scoring categories reduced to **Demand / CSI / TAM** only; the following metrics were removed from spreadsheet + drawer UI as orphans / retired (DB columns preserved): `avg_camp_price_per_hour` (Pricing category retired May 15), `avg_school_rating` (legacy, not in 46-metric approved list), `summer_weather_index` + `avg_peak_summer_temperature` + `days_above_90f` + `summer_precip_days` (Ops weather orphans), `school_district_count` (not in any live category). NULL→0 coercion bug fixed in `cityScoringLiveData.ts` — `population` and `competitorCount` now typed `number | null`; spreadsheet renders NULL as "—" and 0 as "0"; percentile pools exclude NULL rows. Manus owns CSI end-to-end (precomputes `csi_score` + `csi_local_camp_estimate`); we do not seed `summer_camp_count` / `school_hosted_camp_count` from our side. **Additional legacy code purge (same day):** dormant `pricing_power` category branch removed from `MarketDetailDrawer.tsx` + `MarketReportModal.tsx`; weather signal-key overrides removed from `signalGeography.ts`; `avg_hourly_camp_pricing` normalization range removed from `sowNormalize.ts`. **Spreadsheet page-size dropdown expanded to 25/50/100/250/500/All** so the full 817-city list can be reviewed on one page. **`summer_camp_count` / "Camps (count)" column retired from UI** (was 0/817 populated; CSI fully covered by the three Manus-precomputed columns — Nat'l Brand Supply Wtd, Local Provider Est., Demand-Adj. Market — all 817/817). DB column preserved; `MarketDetailDrawer` CSI formula rewritten to describe the real Manus inputs instead of the legacy `competitor_count × 3` placeholder. May 21: v1.2 two-pool architecture shipped — Master Pool vs SmartLead split; `EmailOutreachV2` scope switcher + 6-card stat strip; Master Pool Import Wizard with AI-mapped CSV; Push-to-SmartLead with dry-run.)
...
| `us_cities_scored` | **Canonical city table — 817 rows (Manus universe, locked May 22).** Pre-scored (Task #0). Includes `public_school_count` / `public_school_enrollment` (all open K–12 public schools) and `public_elementary_count` / `public_elementary_enrollment` (derived subset, `lowest_grade_offered ≤ 5`) as cached counts. Retired/orphan columns kept for history but **not read by live UI or Excel export**: `avg_camp_price_per_hour`, `avg_school_rating`, `summer_weather_index`, `avg_peak_summer_temperature`, `days_above_90f`, `summer_precip_days`, `school_district_count`, `school_hosted_camp_count` (Manus-owned CSI input only). Authenticated users can INSERT (Add City flow via `AddCityModal`). |
...
Active limitations:
- **City Search** — `us_cities_scored` locked at **817 cities** (Manus universe). Live categories = Demand / CSI / TAM only. Retired/orphan metric columns preserved in DB but not surfaced in UI (see § 2). NULL rendering contract enforced: NULL → "—", 0 → "0"; NULL rows excluded from percentile pools. Spreadsheet page-size dropdown now offers 25/50/100/250/500/All so the full list can be reviewed on one page. CSI is read exclusively from the three Manus-precomputed columns (`csi_national_brand_count_weighted`, `csi_local_provider_estimate`, `csi_demand_adjusted_market`); `summer_camp_count` / "Camps (count)" is no longer surfaced anywhere in the app (DB column preserved, dormant). **May 22 — 152-city public-school backfill (B-minimal):** the 152 previously-unlinked Manus cities are now FK-linked to `public_schools`; `public_elementary_count`, `charter_elementary_count`, `public_elementary_teacher_count`, and `score_tam_teachers` recomputed via the SOW formula for all 814 cities with linked schools. **`composite_score_default` still reflects the Manus one-time snapshot — it does NOT yet include the refreshed TAM input.** Full 6-category re-score (B-full) deferred pending the Manus competitive-landscape refresh; tracked in OPEN_TASKS.
...
- ~~**Composite-score drift between table and right-panel gauge.**~~ ✅ May 23 — same city was showing `88` in the ranked table and `23` in the side-panel gauge / executive summary because two different files were computing "Score" two different ways. Fix: new `src/lib/marketView.ts` is the single selector layer for every per-market UI number; exposes a branded `CompositeScore` TS type so components can't substitute a raw `number`, plus a dev-mode drift assertion that throws a red `console.error` if the same `(cityId, weightsHash)` mints two different composites in one render. `CityScoring.tsx` now routes the gauge, executive summary, market summary, and formula popover through `selectedView.composite`. AGENTS.md Rule 12 and HOW_IT_WORKS.md § 4 "Score display invariant" added. Snapshot tests on the selector deferred per sprint policy.
