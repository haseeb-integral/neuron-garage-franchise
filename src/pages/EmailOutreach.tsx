...
const replyText = (status: ReplyStatus) => status === "Meeting Booked" ? "Meeting Booked" : status;
  ...
  return (
    <div className="min-h-screen bg-white text-[#07142f]">
      <div className="mb-3 flex items-center justify-between gap-4">
        <div className="relative w-full max-w-[520px]"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#8794ab]" /><input className="h-10 w-full rounded-xl border border-[#dbe4f2] bg-white pl-10 pr-4 text-sm outline-none placeholder:text-[#8794ab]" placeholder="Search campaigns, prospects, schools, cities..." /></div>
        <div className="hidden items-center gap-3 lg:flex"><button className="rounded-full border border-[#e7edf5] p-2 text-[#526078]"><AlertCircle size={18} /></button><div className="flex items-center gap-3 rounded-xl border border-[#e7edf5] px-3 py-2"><div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#174be8] text-sm font-bold text-white">H</div><div className="leading-tight"><div className="text-sm font-bold">Haseeb</div><div className="text-[11px] text-[#66728a]">ADMIN</div></div></div></div>
      </div>

      <div className="mb-3 flex items-start justify-between gap-4">
        <div className="min-w-0"><h1 className="text-[26px] font-black tracking-tight">Email Outreach</h1><p className="mt-1 text-sm text-[#526078]">Manage teacher prospect outreach campaigns and move interested replies into the candidate pipeline.</p></div>
        <div className="ml-auto flex shrink-0 flex-wrap justify-end gap-2 pt-1"><button onClick={() => safeToast("Sample CSV export prepared. Live exports will use synced campaign data.")} className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-[#dbe4f2] bg-white px-3 text-xs font-bold text-[#07142f]"><Download size={14} /> CSV</button><button onClick={() => safeToast("Sample replies synced. SmartLead or GHL sync will connect later.")} className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-[#dbe4f2] bg-white px-3 text-xs font-bold text-[#07142f]"><RefreshCw size={14} /> Sync Replies</button><button onClick={() => safeToast("Campaigns would be imported from SmartLead/GHL in the live version.")} className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-[#dbe4f2] bg-white px-3 text-xs font-bold text-[#174be8]"><Upload size={14} /> Import Campaigns</button><button onClick={() => safeToast("Campaign creation is sample-only until SmartLead/GHL is connected.")} className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-[#174be8] px-3 text-xs font-bold text-white"><Plus size={14} /> Campaign</button></div>
      </div>

      <div className="mb-3 grid gap-2 md:grid-cols-3 xl:grid-cols-6">
        {[[Mail, "Active Campaigns", "4", "Across 4 cities", "blue"], [Users, "Prospects in Outreach", "1,248", "+128 this week", "purple"], [Send, "Open Rate", "42.6%", "+6.3 pp", "green"], [Reply, "Replies", "186", "14.9% reply rate", "green"], [Target, "Interested Leads", "58", "31.2% of replies", "gold"], [Mail, "Promoted to Pipeline", "23", "+8 this week", "blue"]].map(([Icon, label, value, sub, tone]) => <Card key={label as string} className="px-3 py-2.5"><div className="flex items-center gap-2"><IconBox tone={tone as "blue" | "green" | "gold" | "purple"}><Icon size={17} /></IconBox><div className="min-w-0"><div className="truncate text-[11px] font-bold text-[#34445f]">{label as string}</div><div className="text-[21px] font-black leading-6">{value as string}</div><div className="truncate text-[11px] font-bold text-[#0a8f5a]">{sub as string}</div></div></div></Card>)}
      </div>
      ...
      <div className="grid gap-3 xl:grid-cols-[275px_minmax(0,1fr)_280px]">
        ...
        <Card className="overflow-hidden">
          <div className="border-b border-[#edf2f8] p-3">
            ...
            <div className="mt-3 grid grid-cols-3 gap-0 rounded-xl border border-[#edf2f8] bg-white md:grid-cols-6">{[["SENT", selectedCampaign.sent], ["OPEN RATE", selectedCampaign.openRate], ["REPLY RATE", selectedCampaign.replyRate], ["REPLIES", selectedCampaign.replies], ["INTERESTED", selectedCampaign.interested], ["PROMOTED", selectedCampaign.promoted]].map(([label, value]) => <div key={label} className="border-r border-[#edf2f8] px-3 py-2 last:border-r-0"><div className="text-[9px] font-bold text-[#8794ab]">{label}</div><div className={`mt-0.5 text-base font-black ${String(label).includes("RATE") || label === "INTERESTED" ? "text-[#0a8f5a]" : "text-[#07142f]"}`}>{value}</div></div>)}</div>
            <div className="mt-3 grid grid-cols-[1fr_auto_1fr_auto_1fr] items-center rounded-xl border border-[#edf2f8] px-3 py-3">{[["Day 1", "Cold Outreach"], ["Day 3", "Follow-Up 1"], ["Day 7", "Final Follow-Up"]].map((item, idx) => <div key={item[0]} className={`flex items-center gap-2 ${idx === 0 ? "justify-start" : idx === 1 ? "justify-center" : "justify-end"}`}><div className="flex h-8 w-8 items-center justify-center rounded-full border border-[#dbe4f2] bg-[#eef4ff] text-[#174be8]"><Mail size={15} /></div><div><div className="text-xs font-black">{item[0]}</div><div className="text-xs text-[#526078]">{item[1]}</div></div></div>).flatMap((node, idx) => idx < 2 ? [node, <div key={`line-${idx}`} className="h-px border-t border-dashed border-[#cbd5e1]" />] : [node])}</div>
          </div>
