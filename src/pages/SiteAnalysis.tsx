...
import { austinSiteAnalysisDemo, type SiteAnalysisDemoSite } from "@/data/phase2DemoData";
...
function tierBadge(score: number) {
  if (score >= 75) return { bg: "#e3f3e7", fg: "#1d6b32", label: "Recommend" };
  if (score >= 60) return { bg: "#fff8d9", fg: "#7a5800", label: "Worth a look" };
  ...
  return (
    ...
    >
      <div
        ...
        style={{ width: 150, height: 150, border: `1.5px dashed ${BLUE}`, opacity: 0.45 }}
      />
      <div
        className="absolute rounded-full"
        style={{ width: 90, height: 90, border: `2px solid ${BLUE}`, opacity: 0.7 }}
      />
      <div
        className="absolute rounded-full"
        style={{ width: 14, height: 14, backgroundColor: BLUE, boxShadow: "0 0 0 4px rgba(23,75,232,0.15)" }}
      />
      <span
        className="absolute bottom-2 left-2 rounded-md bg-white/90 px-2 py-0.5 text-[10px] font-semibold"
        style={{ color: NAVY }}
      >
...
function SiteCard({ site }: { site: SiteAnalysisDemoSite }) {
  ...
  const rows: { label: string; value: number; weight: number; formula: string }[] = [
        ...
        <div className="mt-2 grid grid-cols-2 gap-2 text-[11px]">
          <div className="rounded-md p-2" style={{ backgroundColor: SOFT }}>
            ...
            <div className="font-bold" style={{ color: NAVY }}>
              {site.isochroneCallouts.medianHHI10min}
            </div>
          </div>
          <div className="rounded-md p-2" style={{ backgroundColor: SOFT }}>
            <div className="text-[9px] uppercase tracking-wide" style={{ color: MUTED }}>
            ...
            <div className="font-bold" style={{ color: NAVY }}>
              {site.isochroneCallouts.pctOver150k10min}
            </div>
          </div>
          <div className="rounded-md p-2" style={{ backgroundColor: SOFT }}>
            <div className="text-[9px] uppercase tracking-wide" style={{ color: MUTED }}>
            ...
            <div className="font-bold" style={{ color: NAVY }}>
              {site.isochroneCallouts.children5to12Within10min}
            </div>
          </div>
          <div className="rounded-md p-2" style={{ backgroundColor: SOFT }}>
            <div className="text-[9px] uppercase tracking-wide" style={{ color: MUTED }}>
            ...
            <div className="font-bold" style={{ color: NAVY }}>
              {site.isochroneCallouts.children5to12Within15min}
            </div>
          </div>
        </div>
      </div>
          ...
          <li key={r.label}>
            ...
            <div className="mt-1 h-1.5 w-full overflow-hidden rounded-full" style={{ backgroundColor: "#eef2f7" }}>
              <div
                ...
                style={{
                  width: `${r.value}%`,
                  backgroundColor: r.value >= 75 ? "#1d6b32" : r.value >= 60 ? "#925100" : "#a3142b",
                }}
              />
            </div>
          </li>
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 max-w-[180px] text-[11px]">
        Up to 4 sites compared side-by-side. Disabled in demo.
      </p>
