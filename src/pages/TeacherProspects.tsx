...
const TeacherProspects = () => {
  ...
  useEffect(() => { (async () => { const emails = prospects.map((p) => p.email).filter(Boolean); if (emails.length === 0) return; const { data } = await supabase.from("candidates").select("email").in("email", emails); if (!data) return; const taken = new Set(data.map((r: any) => r.email)); setPromotedIds(new Set(prospects.filter((p) => taken.has(p.email)).map((p) => p.id))); })(); }, []);

  const handlePromote = async (p: TeacherProspect) => { if (promotedIds.has(p.id) || promotingId === p.id) return; setPromotingId(p.id); const [first_name, ...rest] = p.name.trim().split(/\s+/); const last_name = rest.join(" ") || ""; const { data: inserted, error } = await supabase.from("candidates").insert({ prospect_id: null, first_name, last_name, email: p.email, city: p.city, state: p.state, current_stage: "new_lead", fit_score: p.fitScore ?? 0, fit_tag: deriveFitTag(p.fitScore ?? 0), status: "active", assigned_to: user?.email ?? null }).select("id").single(); if (error || !inserted) { setPromotingId(null); const msg = error?.message ?? "Failed to promote"; if (msg.toLowerCase().includes("duplicate") || msg.includes("unique")) { setPromotedIds((prev) => new Set(prev).add(p.id)); toast.info(`${p.name} is already in the Candidate Pipeline`); } else toast.error(`Could not promote ${p.name}: ${msg}`); return; } await supabase.from("candidate_stage_history").insert({ candidate_id: inserted.id, from_stage: null, to_stage: "new_lead", changed_by: user?.email ?? null, notes: "Promoted from Teacher Prospects" }); setPromotedIds((prev) => new Set(prev).add(p.id)); setPromotingId(null); qc.invalidateQueries({ queryKey: ["candidates"] }); toast.success("Promoted to Candidate Pipeline", { description: `${p.name} is now in New Lead.`, action: { label: "View in pipeline", onClick: () => navigate(`/candidate-pipeline?candidate=${inserted.id}`) } }); };
  ...
  return (
    <div className="-mx-3 -my-3 min-h-screen bg-white px-3 py-3 md:-mx-5 md:px-5 lg:-mx-6 lg:px-6">
      <div className="mx-auto w-full max-w-[1360px]">
        <PageHeader title={selectedMarket ? `${selectedMarket.city}, ${stateAbbr(selectedMarket.state)} Teacher Prospects` : "Teacher Prospects"} subtitle={selectedMarket ? "Discover, evaluate, and promote high-potential teacher-operators for this selected market." : "Discover and evaluate potential franchisee candidates from the teaching community."} hideJourneyBar searchPlaceholder="Search teacher prospects, schools, cities, or specialization..." action={<div className="flex flex-wrap items-center gap-2"><Button size="sm" variant="outline" onClick={handleExport} className="h-9 rounded-lg border-[#dbe4f2] bg-white px-4 text-[#174be8] shadow-none hover:bg-[#f4f7ff]"><Download size={14} /> Export CSV</Button><Button size="sm" onClick={handleRunMarketSearch} className="h-9 rounded-lg bg-[#174be8] px-4 text-white shadow-none hover:bg-[#123fc5]"><Search size={14} /> Find Prospects</Button></div>} />
        {selectedMarket && <section className="mb-3 rounded-xl border border-[#e7edf5] bg-white p-4 shadow-[0_1px_2px_rgba(15,23,42,0.02)]"><div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div><div className="mb-1 text-xs font-black uppercase tracking-wide text-[#174be8]">Market Context</div><div className="mb-1 flex flex-wrap items-center gap-2"><h2 className="text-lg font-black text-[#07142f]">Finding teacher-operators for {selectedMarket.city}, {stateAbbr(selectedMarket.state)}</h2><span className="rounded-full bg-[#e6f7ef] px-2.5 py-1 text-xs font-bold text-[#0a8f5a]">Tier {selectedMarket.tier}</span></div><p className="text-sm text-[#526078]">From City Search · Market Score <strong className="text-[#07142f]">{selectedMarket.compositeScore}</strong> · Population {selectedMarket.population.toLocaleString()} · {selectedMarket.elementarySchools} elementary schools</p></div><div className="flex flex-wrap items-center gap-2"><Button size="sm" variant="outline" className="h-8 rounded-lg border-[#dbe4f2] bg-white px-3 text-[#174be8] shadow-none hover:bg-[#f4f7ff]" onClick={() => navigate("/city-scoring")}>Change Market</Button><Button size="sm" variant="outline" className="h-8 rounded-lg border-[#dbe4f2] bg-white px-3 text-[#174be8] shadow-none hover:bg-[#f4f7ff]" onClick={handleRunMarketSearch}><RefreshCw size={14} /> Run Prospect Search</Button></div></div><div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-5">{[[selectedMarket.elementarySchools,"Schools Found",<School size={15} />],[Math.max(marketProspects.length, selectedMarket.elementarySchools * 12),"Teacher Pool",<Users size={15} />],[enrichedCount,"Enriched Contacts",<Sparkles size={15} />],[highFitCount,"High-Fit Teachers",<TrendingUp size={15} />],[`${clusterPotential}-${Math.min(5,(clusterPotential??1)+1)}`,"Cluster Potential",<BarChart3 size={15} />]].map(([v,l,i],idx)=><div key={String(l)} className="flex items-center gap-3 rounded-xl border border-[#edf2f8] bg-white p-3"><div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${idx===4?"bg-[#e6f7ef] text-[#0a8f5a]":"bg-[#eaf0ff] text-[#174be8]"}`}>{i as React.ReactNode}</div><div><div className="text-xl font-black leading-none text-[#07142f]">{v as React.ReactNode}</div><div className="mt-1 text-xs font-medium text-[#66728a]">{l as React.ReactNode}</div></div></div>)}</div></section>}
        <div className="mb-3 grid gap-3 lg:grid-cols-4"><MetricCard title="Total Prospects" value="354" delta="25% vs last period" icon={<Users size={18} />} tone="cyan" /><MetricCard title="Shortlisted" value="86" delta="18% vs last period" icon={<Star size={18} />} tone="gold" /><MetricCard title="Contacted This Week" value="42" delta="31% vs last period" icon={<Mail size={18} />} tone="blue" /><MetricCard title="Promoted to Pipeline" value="23" delta="21% vs last period" icon={<ArrowRight size={18} />} tone="green" action={<button className="text-[11px] font-bold text-[#174be8]">View All</button>} /></div>
        <div className="grid items-start gap-3 xl:grid-cols-[minmax(0,1fr)_320px]"><div className="min-w-0 space-y-3"><TeacherFilterBar cities={cities} cityFilter={cityFilter} setCityFilter={setCityFilter} tagFilter={tagFilter} setTagFilter={setTagFilter} gradeFilter={gradeFilter} setGradeFilter={setGradeFilter} campOnly={campOnly} setCampOnly={setCampOnly} enrichmentFilter={enrichmentFilter} setEnrichmentFilter={setEnrichmentFilter} search={search} setSearch={setSearch} /><BulkActionBar count={selected.length} onExport={() => toast.success(`Exported ${selected.length} prospects to CSV`)} onAddTag={() => toast.info("Add tag dialog (placeholder)")} onPromote={() => { toast.success(`Promoted ${selected.length} prospects`); setSelected([]); }} onClear={() => setSelected([])} /><TeacherTable prospects={filtered} selected={selected} onToggleSelect={toggleSelect} onToggleAll={toggleAll} onRowClick={setActive} onPromote={handlePromote} promotedIds={promotedIds} promotingId={promotingId} /></div><SourcingInsights /></div>
