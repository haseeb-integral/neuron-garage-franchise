...
import { ArrowRight, BarChart3, Download, Globe2, Linkedin, Mail, RefreshCw, Search, School, Sparkles, Star, TrendingUp, Upload, UserPlus, Users } from "lucide-react";
  ...
  return (
    <div className="-mx-3 -my-3 min-h-screen bg-white px-3 py-3 md:-mx-5 md:px-5 lg:-mx-6 lg:px-6">
      <div className="mx-auto w-full max-w-[1360px]">
        <PageHeader title={selectedMarket ? `${selectedMarket.city}, ${stateAbbr(selectedMarket.state)} Teacher Search` : "Teacher Search"} subtitle={selectedMarket ? "Discover, evaluate, and send high-potential teacher-operators into outreach for this selected market." : "Discover and evaluate potential franchisee candidates from the teaching community."} hideJourneyBar searchPlaceholder="Search teacher prospects, schools, cities, or specialization..." action={<div className="flex flex-wrap items-center gap-2"><Button size="sm" variant="outline" onClick={handleExport} className="h-9 rounded-lg border-[#dbe4f2] bg-white px-4 text-[#174be8] shadow-none hover:bg-[#f4f7ff]"><Download size={14} /> Export CSV</Button><Button size="sm" variant="outline" onClick={() => setImportOpen(true)} className="h-9 rounded-lg border-[#dbe4f2] bg-white px-4 text-[#174be8] shadow-none hover:bg-[#f4f7ff]"><Upload size={14} /> Import CSV</Button><Button size="sm" onClick={handleRunMarketSearch} className="h-9 rounded-lg bg-[#174be8] px-4 text-white shadow-none hover:bg-[#123fc5]"><Search size={14} /> Find Prospects</Button></div>} />
        ...
        <TeacherImportWizard open={importOpen} onClose={() => setImportOpen(false)} onComplete={loadProspects} />
