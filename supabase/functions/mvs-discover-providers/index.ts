...
// ----------------- Phase 4.1: Tavily lead search (Boston pilot) -----------------
// For missing-price providers, ask Tavily for the best provider-owned camp/
// pricing/tuition URL, then Firecrawl-verifies the price using the same
// literal-match guard as Phase 3.3. Tavily's AI summary is NEVER trusted as
// proof — only the scraped page text counts. Pilot is gated on
// body.tavilyPilot === true AND city === "Boston, MA".

const TAVILY_API = "https://api.tavily.com/search";
const TAVILY_TIMEOUT_MS = 20_000;
const TAVILY_PILOT_CITY = "Boston, MA";
const TAVILY_PILOT_LIMIT = 10;
...
function hostnameOf(u: string | null | undefined): string | null {
  if (!u) return null;
...
type TavilyPilotEntry = {
  ...
  tavily_top_urls: Array<{ url: string; title: string | null; host: string | null }>;
  tavily_credits_used: number;
  picked_url: string | null;
  picked_reason: string;
  firecrawl_scraped: boolean;
  firecrawl_calls_used: number;
  screenshot_path: string | null;
  price_min: number | null;
  price_max: number | null;
  guard_result: "kept" | "dropped" | "no_price" | "skipped";
  guard_drop_reason: string | null;
  snippet_around_price: string | null;
  extraction_method: "tavily_lead_v1";
  error: string | null;
  saved_to_provider: boolean;
};

async function runTavilyPilotForBoston(args: {
  city: string;
  admin: ReturnType<typeof createClient>;
  tavilyKey: string;
  firecrawlKey: string;
  ...
  entries: TavilyPilotEntry[];
  tavilyCalls: number;
  firecrawlCalls: number;
  updated: number;
  debug: Record<string, unknown>;
}> {
  const { city, admin, tavilyKey, firecrawlKey, lovableKey, runId } = args;
  const debug: Record<string, unknown> = {
    city,
    pilot_limit: TAVILY_PILOT_LIMIT,
    started_at: new Date().toISOString(),
  };

  // Pull missing-price providers for Boston. Premium tier first, then fill.
  const { data: premiumRows, error: prErr } = await admin
    .from("mvs_providers")
    .select("id, name, website_url, url, source_listing_url, tier")
    .eq("city", city)
    .is("price_min", null)
    .is("price_max", null)
    .eq("tier", "premium")
  ...
  if (prErr) {
    debug.error = `fetch premium: ${prErr.message}`;
    return { entries: [], tavilyCalls: 0, firecrawlCalls: 0, updated: 0, debug };
  }
  let candidates: TavilyPilotProvider[] = (premiumRows ?? []) as TavilyPilotProvider[];

  if (candidates.length < TAVILY_PILOT_LIMIT) {
    const fillCount = TAVILY_PILOT_LIMIT - candidates.length;
  ...
  if (candidates.length === 0) {
    debug.skipped_reason = "no missing-price providers in Boston";
    return { entries: [], tavilyCalls: 0, firecrawlCalls: 0, updated: 0, debug };
  }

  const cleanCity = city.replace(/,\s*[A-Za-z]{2}\s*$/i, "").trim();

  // 10x parallel — each ~6-8s, well under the 150s edge timeout.
  const entries = await Promise.all(candidates.map(async (p): Promise<TavilyPilotEntry> => {
    const providerDomain = hostnameOf(p.website_url) || hostnameOf(p.url);
    const query = `${p.name} ${cleanCity} camp tuition price per week`;
    const entry: TavilyPilotEntry = {
      provider_id: p.id,
      ...
      picked_url: null,
      picked_reason: "none",
      firecrawl_scraped: false,
      firecrawl_calls_used: 0,
      screenshot_path: null,
      price_min: null,
      price_max: null,
      guard_result: "skipped",
      guard_drop_reason: null,
      snippet_around_price: null,
      extraction_method: "tavily_lead_v1",
      error: null,
    ...
    try {
      ...
      if (providerDomain) body.include_domains = [providerDomain];
      ...
      entry.tavily_credits_used = 2; // advanced search depth
      const tj = await res.json().catch(() => ({}));
      if (!res.ok) {
        entry.error = `tavily ${res.status}: ${JSON.stringify(tj).slice(0, 200)}`;
        return entry;
      }
      entry.tavily_answer = typeof tj?.answer === "string" ? tj.answer : null;
      tavilyResults = Array.isArray(tj?.results) ? tj.results : [];
      entry.tavily_top_urls = tavilyResults.slice(0, 5).map((r) => ({
        url: String(r.url ?? ""),
        title: r.title ?? null,
        host: hostnameOf(String(r.url ?? "")),
      }));

      // Retry once without include_domains if provider-domain filter empty.
      if (tavilyResults.length === 0 && providerDomain) {
        const retryBody = { ...body };
        delete (retryBody as Record<string, unknown>).include_domains;
        const res2 = await fetchWithTimeout(TAVILY_API, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(retryBody),
        }, TAVILY_TIMEOUT_MS);
        entry.tavily_credits_used += 2;
        const tj2 = await res2.json().catch(() => ({}));
        if (res2.ok) {
          if (typeof tj2?.answer === "string" && !entry.tavily_answer) entry.tavily_answer = tj2.answer;
          tavilyResults = Array.isArray(tj2?.results) ? tj2.results : [];
          entry.tavily_top_urls = tavilyResults.slice(0, 5).map((r) => ({
            url: String(r.url ?? ""),
            title: r.title ?? null,
            host: hostnameOf(String(r.url ?? "")),
      ...
      entry.error = `tavily exception: ${e instanceof Error ? e.message : String(e)}`;
      return entry;
    }
    ...
    let reason = "none";
    if (providerDomain) {
      const own = tavilyResults.find((r) => {
        const h = hostnameOf(r.url);
        return h === providerDomain || (h !== null && h.endsWith(`.${providerDomain}`));
      });
      if (own) { picked = own.url; reason = "provider_domain"; }
    }
    if (!picked) {
      const nonAgg = tavilyResults.find((r) => !isAggregatorHost(hostnameOf(r.url)));
      if (nonAgg) { picked = nonAgg.url; reason = "first_non_aggregator"; }
    }
    entry.picked_url = picked;
    entry.picked_reason = reason;
    if (!picked) {
    ...
    try {
      ...
      entry.firecrawl_calls_used = 1;
      entry.firecrawl_scraped = true;
      const j = await res.json().catch(() => ({}));
      if (!res.ok) {
        entry.error = `firecrawl ${res.status}`;
        entry.guard_result = "skipped";
        entry.guard_drop_reason = `firecrawl error ${res.status}`;
        return entry;
      }
      md = String(j?.data?.markdown ?? "");
      const shot: string | undefined = j?.data?.screenshot ?? j?.data?.screenshotUrl;
      if (shot) {
        try {
          let bytes: Uint8Array | null = null;
          ...
          if (bytes) {
            const path = `${runId}/tavily-${citySlug(city)}-${p.id}.png`;
            const { error: upErr } = await admin.storage
              .from(SCREENSHOT_BUCKET)
              .upload(path, bytes, { contentType: "image/png", upsert: true });
            if (!upErr) shotPath = path;
      ...
      entry.screenshot_path = shotPath;
    } catch (e) {
      entry.error = `firecrawl exception: ${e instanceof Error ? e.message : String(e)}`;
      entry.guard_result = "skipped";
      entry.guard_drop_reason = "firecrawl exception";
      return entry;
    }

    if (!md) {
      entry.guard_result = "skipped";
      entry.guard_drop_reason = "empty markdown from firecrawl";
      return entry;
    }

    // ---- 4. Gemini extract (strict, no inference) ----
    const sys = `You extract per-week or per-session pricing for the kids' camp/class provider "${p.name}" from a single source page in ${city}.
Return strict JSON: { "providers": [ { "name": string, "url": string|null, "price_min": number|null, "price_max": number|null, "category_raw": string|null, "confidence": number } ] }
${PRICE_RULES}
- Only return prices that belong to "${p.name}" specifically. Skip prices for other businesses mentioned on the page.
- If no dollar amount for this provider appears literally on the page, return providers: [].
- Hard cap: 5 providers.`;

    const gemDebug: Record<string, unknown> = {};
    const found = await extractWithGemini(
      { lovableKey, sys, city, sourceUrl: picked, markdown: md },
      gemDebug,
    );

    const priced = found.filter((f) => f.price_min != null || f.price_max != null);
    if (priced.length === 0) {
      entry.guard_result = "no_price";
      entry.guard_drop_reason = "no literal dollar amount for this provider on page";
      return entry;
    }
    priced.sort((a, b) => (b.confidence ?? 0) - (a.confidence ?? 0));
    const best = priced[0];
    const val = (best.price_min ?? best.price_max) as number;

    // ---- 5. Literal guard: ±150 chars near pricing words ----
    const rx = new RegExp(`\\$\\s?${val}\\b`);
    const m = md.match(rx);
    let snippet = "";
    if (m && m.index != null) {
    ...
    const contextOk =
      /camp|tuition|week|session|class|program|fee|registration|enroll/i.test(snippet);
    entry.snippet_around_price = snippet || null;

    if (!contextOk) {
      entry.guard_result = "dropped";
      entry.guard_drop_reason = "snippet did not mention camp/tuition/week/session/fee/registration context";
      return entry;
    }

    entry.price_min = best.price_min ?? null;
    entry.price_max = best.price_max ?? null;
    entry.guard_result = "kept";

    // ---- 6. Save to provider row ----
    try {
  ...
  const tavilyCalls = entries.reduce((s, e) => s + Math.ceil((e.tavily_credits_used || 0) / 2), 0);
  const firecrawlCalls = entries.reduce((s, e) => s + e.firecrawl_calls_used, 0);
  const updated = entries.filter((e) => e.saved_to_provider).length;

  debug.finished_at = new Date().toISOString();
  debug.entries_total = entries.length;
  debug.entries_kept = entries.filter((e) => e.guard_result === "kept").length;
  debug.entries_dropped = entries.filter((e) => e.guard_result === "dropped").length;
  debug.entries_no_price = entries.filter((e) => e.guard_result === "no_price").length;
  debug.entries_skipped = entries.filter((e) => e.guard_result === "skipped").length;
  debug.tavily_credits_total = entries.reduce((s, e) => s + e.tavily_credits_used, 0);
  debug.firecrawl_calls_total = firecrawlCalls;
  debug.updated = updated;

  return { entries, tavilyCalls, firecrawlCalls, updated, debug };
}
