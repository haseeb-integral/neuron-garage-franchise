// Phase 3 / Turn 3.2 — mvs-extract-weeks-austin-all
//
// Per the approved Feature 1A Build Plan, Turn 3.2:
//   - Iterate ALL Austin Premium providers (platform='sawyer', tier='premium').
//   - For each, run the same Firecrawl scrape + Gemini week extraction as Turn 3.1
//     (kept inline here so Turn 3.1's single-provider function stays untouched
//      and reversible per the atomic-turn rule).
//   - Track "% with no public registration page" = providers whose Sawyer URL is
...
const LOW_CONFIDENCE_BADGE_PCT = 0.20; // >20% no-reg-page → badge
const MAX_PROVIDERS = 25;
const AUSTIN_CITY = "Austin, TX";
const AUSTIN_STATE = "TX";
...
type ProviderOutcome = {
  ...
  url: string | null;
  no_reg_page: boolean;       // missing URL OR scrape empty/failed
...
Deno.serve(async (req) => {
  ...
  // Pull all Austin Premium providers (Sawyer).
  ...
  if (providerList.length === 0) {
    return new Response(
      JSON.stringify({ error: "no Austin Premium providers found (run Turn 2.1/2.2 first)" }),
  ...
  const { data: run, error: runErr } = await admin
    ...
    .insert({ city: AUSTIN_CITY, status: "running", firecrawl_calls: 0 })
    .select()
    .single();
  if (runErr || !run) {
    return new Response(
  ...
  try {
    // Sequential: keeps Firecrawl cost predictable and avoids hammering Sawyer.
    for (const p of providerList) {
      const { outcome, firecrawlCalls: used } = await processProvider(
        admin, p, run.id, firecrawlKey, lovableKey, todayISO,
      );
      outcomes.push(outcome);
      firecrawlCalls += used;
    }

    // Low-confidence city badge: >20% of providers have no public registration page.
    ...
    // Upsert mvs_city_flags for Austin (unique on city+state).
